...
oled.text('IP:'+str(ServerIP),0,0)
oled.text('PORT: '+str(ServerPort),0,16)
oled.show()
...
while True:
    ...
    oled.text('Message RECEIVED: ',0,26)
    oled.text(str(messageDecoded),16,36)
    oled.text('From:',0,46)
    #Given address by that UDPServer.recfrom method
    oled.text(str(address[0]),16,56)
    oled.show()
    oled.fill(0)
